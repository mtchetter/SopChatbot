...
# Adds: Lookup of doclink from Excel listing
...
EXCEL_LIST = r"C:\Users\mtschetter\Desktop\all_sop\SOP Directory.xlsx"
ARCHIVE_SUBFOLDER = "Archive"

PINECONE_INDEX_NAME = "hr-data"

# API keys
PINECONE_API_KEY = "pcsk_7NdY6A_AaELQcsyWS531xhiWsJCqsSrdtT3GHvkWwGR49pB6SEzwThJWzAMTLNkYovRuM4"
OPENAI_API_KEY="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"
...
def load_doclink_mapping(excel_path: str) -> Dict[str, str]:
    """Reads Excel: column B = file name, column D = doclink."""
    wb = load_workbook(excel_path, data_only=True)
    ws = wb.active
    mapping = {}
    for row in ws.iter_rows(min_row=2):  # start row 2
        name_cell = row[1]  # column B (0-based index 1)
        link_cell = row[3]  # column D (0-based index 3)
        if name_cell.value and link_cell.value:
            # Ensure consistent casing/spacing
            mapping[str(name_cell.value).strip()] = str(link_cell.value).strip()
    wb.close()
    return mapping
...
def process_all_docx_and_archive() -> bool:
    ...
    # Load Excel mapping
    log(f"Loading doclink mapping from: {EXCEL_LIST}")
    doclink_map = load_doclink_mapping(EXCEL_LIST)
    log(f"Loaded {len(doclink_map)} Excel entries")
    ...
    active_indexes = [i["name"] for i in raw_indexes]  # <-- new
    ...
    for docx_count, docx_path in enumerate(docx_files, 1):
        ...
        # lookup doclink from Excel mapping (without extension if needed)
        base_name = os.path.splitext(docx_name)[0]
        doclink = doclink_map.get(base_name)
        if not doclink:
            log(" WARNING: No doclink found in Excel for this file, skipping.")
        ...
        for start in range(0, len(chunks), BATCH_SIZE):
            ...
            if vectors_to_upsert:
                try:
                    ...
                    log(f" Upserted {len(vectors_to_upsert)} vectors ({total_upserted}/{len(chunks)})")
                except Exception as e:
